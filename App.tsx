...
import { classifyScrap } from "./api/classifier";
...
const MATS: Mat[] = ["Iron", "Copper", "Aluminium", "Steel", "Brass"];
const MCOL: Record<Mat, string> = { Iron: "#8b9096", Copper: "#f08a3c", Aluminium: "#e9f2ed", Steel: "#4da3ff", Brass: "#e6c04a", "Stainless Steel": "#a8b0b8", Plastic: "#5cb85c", "Mixed Scrap": "#9aa0a6" };
const PAL: Record<string, string[]> = {
  ...
  Iron: ["#697078", "#343a40", "#a0a6ad", "#242a30"],
};
...
const detectScrap = async (file: File | null, kg: number, _depth: Depth): Promise<Det[]> => {
  const response = await classifyScrap(file, kg);
  const classification = response.classification || response.fallback;
  if (!classification) throw new Error(response.error || "The classifier returned no result.");
  return classification.composition.map((component) => ({
    material: component.material,
    qty: Math.round(kg * component.percentage / 100),
    grade: component.grade,
    conf: Math.round(component.confidence * 100),
    purity: 100 - classification.contamination_percent,
  }));
};
...
function Scan({ buyers, batches, onAdd, go }: { buyers: Buyer[]; batches: Batch[]; onAdd: (x: NewBatch[]) => void; go: (p: Page) => void }) {
  ...
  const run = async () => {
    ...
    setScanError("");
    try {
      setRes(await detectScrap(file, kg, depth));
    } catch (error) {
    ...
    <div className="g2">
      ...
      <div className="stack">
        <div className="card">
          ...
          {scanError && <p className="mut" style={{ color: "#ff8b8b", marginTop: 10 }}>{scanError}</p>}
...
export default function App() {
  ...
  const batches: Batch[] = useMemo(() => {
    ...
    return dbBatches.map(b => ({
      ...
      grade: b.grade,
      qty: b.quantity,
      buyer: b.buyer_id ? buyerMap[b.buyer_id] || "—" : "—",
      value: b.value,
